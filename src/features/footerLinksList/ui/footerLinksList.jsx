	...
	return (
		<div className={classes.footer__links}>
			{links.slice(0, 2).map((link, i) => (
				<Link key={`footerLink${i}`} to={link.href} className={classes.footer__linksLink}>{link.text}</Link>
			))}
			{links.length > 2 ? 
				<button
				...
				>
					...
					>
						{links.slice(2).map((link, i) => (
							<Link key={`hiddenLink${i}`} to={link.href} className={classes.hiddenLinksLink}>{link.text}</Link>
						))}
					</div>
				</button>
			: ""
